...
export default function JobQueue() {
  ...
  async function load() {
    setRefreshing(true);
    try {
      const jobsRes = await axios.get(`${API_URL}/ai/jobs/queue`);
      const invRes = await axios.get(`${API_URL}/ai/inventory`);
  ...
  function togglePart(itemId: string) {
    setSelectedParts(function (prev) {
  ...
  function changeQty(itemId: string, delta: number) {
    setSelectedParts(function (prev) {
      const current = prev[itemId] != null ? prev[itemId] : 1;
      ...
      <Modal visible={completeModalVisible} animationType="slide" transparent={true}>
        <View style={s.modalOverlay}>
          <View style={s.modalCard}>
                ...
                return (
                  <View key={item.id} style={s.partRow}>
                    <TouchableOpacity style={s.partCheckRow} onPress={() => togglePart(item.id)}>
                      <View style={[s.checkbox, selected ? s.checkboxChecked : null]} />
                    ...
                    {selected && (
                      <View style={s.qtyControls}>
                        ...
                        <Text style={s.qtyValue}>{selectedParts[item.id]}</Text>
                        <TouchableOpacity style={s.qtyBtn} onPress={() => changeQty(item.id, 1)}>
                          <Text style={s.qtyBtnText}>+</Text>
                        </TouchableOpacity>
                      </View>
            ...
            <View style={s.modalActions}>
              <TouchableOpacity style={s.modalCancelBtn} onPress={() => setCompleteModalVisible(false)}>
                <Text style={s.modalCancelText}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity style={s.modalSaveBtn} onPress={confirmComplete}>
                <Text style={s.modalSaveText}>Complete Job</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
...
const s = StyleSheet.create({
  ...
  modalCard: { backgroundColor: "#111827", borderRadius: 12, padding: 20, borderWidth: 1, borderColor: "#1e2d40", maxHeight: "80%" },
  ...
  partsList: { maxHeight: 260 },
  partRow: { paddingVertical: 8, borderBottomWidth: 1, borderBottomColor: "#1a2740" },
  partCheckRow: { flexDirection: "row", alignItems: "center", gap: 10 },
  checkbox: { width: 18, height: 18, borderRadius: 4, borderWidth: 2, borderColor: "#475569" },
  checkboxChecked: { backgroundColor: "#3b82f6", borderColor: "#3b82f6" },
  partName: { flex: 1, color: "#e2e8f0", fontSize: 13 },
  qtyControls: { flexDirection: "row", alignItems: "center", gap: 12, marginTop: 8, marginLeft: 28 },
  qtyBtn: { backgroundColor: "#1e293b", width: 28, height: 28, borderRadius: 6, alignItems: "center", justifyContent: "center" },
  qtyBtnText: { color: "#e2e8f0", fontSize: 16, fontWeight: "700" },
  qtyValue: { color: "#e2e8f0", fontSize: 14, fontWeight: "600", minWidth: 20, textAlign: "center" },
});